...
parser.add_argument('--path', default='/home/maomao/alg/dataset/', help='path of datasets')
parser.add_argument('--dataset', default='diginetica-2', help='Tmall-2/deginetica-2/retailrocket-2')
...
parser.add_argument('--layer', type=float, default=3, help='model depth')
parser.add_argument('--k', type=float, default=20, help='number of the random walk step')
parser.add_argument('--threshold', type=float, default=0.0, help='standard of decoupling')
parser.add_argument('--p', type=float, default=0.15, help='probability of item masking')
parser.add_argument('--lambda_alpha', type=float, default=0.00001, help='scale of Lap loss')
parser.add_argument('--theta1', type=float, default=0.1, help='scale of same-scale contrastive learning')
parser.add_argument('--theta2', type=float, default=0.2, help='scale of cross-scale contrastive learning')
parser.add_argument('--temp1', type=float, default=0.6, help='temperature for same-scale contrastive learning')
parser.add_argument('--temp2', type=float, default=0.1, help='temperature for cross-scale contrastive learning')
...
print('the number of the Random Walk step is is :{}'.format(opt.k))
...
train_data = Data(opt, train_data, adj_freq, length)
test_data = Data(opt, test_data, adj_freq, length)
max_len = train_data.__len__()

# ----------------- define the model ------------------ #

model = trans_to_cuda(DDEAN(opt, num_node))
#saved = torch.load('/home/dtt/wangx/dataset/'+opt.dataset+'saved/' + 'ddgnn_dg.pth')
...
for epoch in range(opt.epoch):
    ...
    if best_result < recall:
        ...
        state_dict = {"model": model.state_dict(), "embedding": model.embedding.state_dict(),
                      'posembedding': model.pos_embedding.state_dict()}
        torch.save(state_dict, '/home/maomao/alg/saved/' + 'saved_DDGNN_dg_Lp.pth')
