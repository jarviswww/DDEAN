...
def structuralRep(A, max_n_node, dim=16):
    adj_dgl = adj_to_dgl(A, type_graph='dgl')
    position_embed = dgl.random_walk_pe(adj_dgl, dim)  # Num_items x 16
    position_embed = torch.cat((position_embed, torch.zeros((max_n_node - len(position_embed), dim))), 0)  # len x 16
    return position_embed

class LapLoss(nn.Module):
    def __init__(self, k, lam_alpha):
    ...
    def forward(self, H, L):
        ...
        for i in range(len(H)):
            x = torch.mm(torch.mm(H[i].transpose(1, 0), L[i]), H[i])
            lp_loss = torch.trace(x)

            identity_matrix = torch.eye(self.k).cuda()
            ptp = torch.mm(H[i].t(), H[i])
            frobenius_norm_term = torch.norm(ptp - identity_matrix, 'fro')
            frobenius_loss = (frobenius_norm_term ** 2)

            loss += lp_loss + self.lam_alpha * frobenius_loss
...
class Data(Dataset):
    def __init__(self, opt, data, adj_global, train_len=None):
        self.data = data
    ...
    def __getitem__(self, index):
        ...
        position_embed = dgl.random_walk_pe(adj_dgl, self.k)  # Num_items x 16
        position_embed = torch.cat((position_embed, torch.zeros((max_n_node - len(position_embed), self.k))), 0)  # len x 16
        ...
        return [torch.tensor(target), torch.tensor(u_input), torch.tensor(adj),
                torch.tensor(adj_nd),
                torch.tensor(session_items), torch.tensor(u_input_index), position_embed]
